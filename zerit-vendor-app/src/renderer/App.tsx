...
export default function App() {
  ...
  const GetOrder = async () => {
    ...
    try {
      const data: ApiResponse = await window.electronAPI.fetchData(token);
      if (data.success) {
        setOrder(data.order);
  ...
  const handlePrint = (file: FileItem) => {
    window.electronAPI.printPDF(file.url, file.colorType, file.printType)
    setPrintedFiles((prev) => new Set(prev).add(file.id));
  };

  const handlePrintAll = () => {
    if (!order) return;
    order.files.map((f) => window.electronAPI.printPDF(f.url))
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <button
                          onClick={() => handlePrint(file.id, file.url)}
                          disabled={isPrinted}
                          className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-all duration-200 ${
                            isPrinted
                              ? "bg-green-100 text-green-600 dark:bg-green-500/15 dark:text-green-400 cursor-default"
                              : "bg-cyan-500 text-white hover:bg-cyan-600 active:scale-95"
                          }`}
                        >
                          {isPrinted ? "Done ✓" : "Print"}
                        </button>
                      </div>
                    </div>
                  );
